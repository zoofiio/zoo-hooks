...
import {ZooCustomAccountingMock} from "test/mocks/ZooCustomAccountingMock.sol";
...
contract BaseCustomAccountingTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        deployCodeTo(
            "test/mocks/ZooCustomAccountingMock.sol:ZooCustomAccountingMock", abi.encode(manager), address(hook)
        );
    ...
    function test_addLiquidity_native_succeeds() public {
        ZooCustomAccountingMock nativeHook = ZooCustomAccountingMock(0x1000000000000000000000000000000000002A00);
        deployCodeTo(
            "test/mocks/ZooCustomAccountingMock.sol:ZooCustomAccountingMock", abi.encode(manager), address(nativeHook)
        );
        (key, id) = initPool(
    ...
    function test_addLiquidity_native_invalidValue_revert() public {
        ...
        vm.expectRevert(ZooCustomAccounting.InvalidNativeValue.selector);
    ...
    function test_removeLiquidity_initialRemove_succeeds() public {
        ...
        ZooCustomAccounting.RemoveLiquidityParams memory removeLiquidityParams =
            ZooCustomAccounting.RemoveLiquidityParams(1 ether, 0, 0, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0));
    ...
    function test_removeLiquidity_fuzz_succeeds(uint256 amount) public {
        ...
        if (amount > hook.balanceOf(address(this))) {
            vm.expectRevert();
            hook.removeLiquidity(
                ZooCustomAccounting.RemoveLiquidityParams(amount, 0, 0, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0))
            );
        } else {
    ...
    function test_removeLiquidity_partial_succeeds() public {
        ...
        hook.removeLiquidity(
            ZooCustomAccounting.RemoveLiquidityParams(5 ether, 0, 0, MAX_DEADLINE, MIN_TICK, MAX_TICK, bytes32(0))
        );
    ...
    function test_removeLiquidity_notInitialized_reverts() public {
        ZooCustomAccountingMock uninitializedHook =
            ZooCustomAccountingMock(0x1000000000000000000000000000000000002A00);
        deployCodeTo(
            "test/mocks/ZooCustomAccountingMock.sol:ZooCustomAccountingMock",
            abi.encode(manager),
            address(uninitializedHook)
        );

        vm.expectRevert(ZooCustomAccounting.PoolNotInitialized.selector);
    ...
    function test_beforeInitialize_alreadyInitialized_reverts() public {
        ...
        vm.expectRevert(ZooCustomAccounting.AlreadyInitialized.selector);
